...
get_co2() {
  ...
  psql -h "$PGRES_HOST" -p "$PGRES_PORT" -U "$PGRES_USER" -f schemas_n_data_co2.sql
...
get_population() {
  ...
  POPUL="WPP2019_TotalPopulationBySex.csv"
  POPUL_SHEET="population_by_sex.csv"

  # Download UN Population Dataset
  if [[ ! -f "$POPUL" ]]; then
    echo "Downloading $POPUL..."
    wget "https://population.un.org/wpp/Download/Files/1_Indicators%20(Standard)/CSV_FILES/WPP2019_TotalPopulationBySex.csv"
  else
    echo "$POPUL has already been downloaded"
  fi

  # Correct the CSVs
  tail -n +2 $POPUL > $POPUL_SHEET

  # Load the Schemas and Data
  psql -h "$PGRES_HOST" -p "$PGRES_PORT" -U "$PGRES_USER" -f schemas_n_data_population.sql
